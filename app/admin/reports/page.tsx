    ...
    <div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(190px,1fr))", gap: "1rem", marginBottom: "2rem" }}>
        {[
          ...
          <div key={kpi.label} className="card" style={{ padding: "1.25rem" }}>
            <div style={{ fontSize: "1.625rem", fontWeight: 800, color: "var(--navy)", letterSpacing: "-0.02em", marginBottom: "0.25rem" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem", marginBottom: "1.5rem" }}>
        ...
        <div className="card" style={{ padding: "1.75rem" }}>
          ...
          {(data?.revenueMonthly ?? []).length === 0 ? (
            <div style={{ color: "var(--gray-400)", fontSize: "0.9rem", padding: "1rem 0" }}>No revenue data yet — data will appear after first payments.</div>
          ) : (
              ...
              {(data?.topStates ?? []).map((row, i, arr) => (
                <tr key={i} style={{ borderBottom: i < arr.length - 1 ? "1px solid var(--gray-50)" : "none" }}>
                  ...
                  <td style={{ padding: "0.875rem 0", minWidth: "140px" }}>
                    <Bar pct={(row.contractors / maxStateContractors) * 100} />
                  </td>
                </tr>
              ))}
            </tbody>
      ...
      <div className="card" style={{ padding: "1.75rem" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(200px,1fr))", gap: "1rem" }}>
          {[
            { label: "Active Contractors", value: `${s?.activeContractors ?? 0}`, status: "good" },
            { label: "Contractor Activation Rate", value: s && s.totalContractors > 0 ? `${Math.round((s.activeContractors / s.totalContractors) * 100)}%` : "—", status: "good" },
            { label: "Total Quote Requests", value: s?.totalQuotes.toLocaleString() ?? "0", status: "good" },
            { label: "Reviews Published", value: s?.totalReviews.toLocaleString() ?? "0", status: "good" },
          ].map(item => (
